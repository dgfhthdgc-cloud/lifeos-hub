...
import { Award, Zap, ChevronRight, Sparkles, Shield, Flame } from 'lucide-react';
...
export function LevelProgressHero({ user, onOpenRankMatrix }: LevelProgressHeroProps) {
  const xpPercent = Math.min(
    100,
    Math.round((user.currentXp / (user.nextLevelXp || 1000)) * 100)
  );
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-3 max-w-xl">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-neutral-300 font-medium">Rank Stage {Math.floor(user.level / 5) + 1}</span>
          </div>
          ...
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between text-xs font-mono">
              ...
              <span className="text-emerald-400 font-bold">
                {user.currentXp.toLocaleString()} / {user.nextLevelXp.toLocaleString()} XP ({xpPercent}%)
              </span>
            </div>
            <Progress value={xpPercent} className="h-2.5 bg-neutral-800" />
            <p className="text-[11px] text-neutral-400">
              {(user.nextLevelXp - user.currentXp).toLocaleString()} XP required to reach Level {user.level + 1}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
          <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-neutral-400 uppercase font-semibold">Total Level Perks</p>
              <p className="text-sm font-bold text-white">{user.level * 2} Unlocked</p>
            </div>
          </div>
